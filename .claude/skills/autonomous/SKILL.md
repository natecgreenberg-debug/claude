...
- **Heartbeat**: At the start of Phase 4, start a background heartbeat loop: `while true; do date -u > ~/projects/Agent/autonomous_runs/{NNN}_{slug}/heartbeat; sleep 60; done &` and save the PID. Kill the heartbeat process at the end of Phase 4 (before entering Phase 5): `kill $HEARTBEAT_PID 2>/dev/null || true`. This lets the external watchdog (`tools/watchdog.sh`) detect stalled runs.
...
- **2026-03-10**: Added heartbeat mechanism — Phase 4 writes UTC timestamp to heartbeat file every 60s, enabling external watchdog monitoring via `tools/watchdog.sh`
